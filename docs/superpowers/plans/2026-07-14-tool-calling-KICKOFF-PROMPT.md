...
- Task 12 step 3 is a **blocking user-validation gate**: the release is validated by running the example app's tool-calling demo against a real provider. If `example/.env` has credentials, run `npm run tool-demo` yourself; if not, stop and ask the user to either add credentials or run it themselves (they can type `! cd example && npm run tool-demo` in the session so the output lands in the conversation). The plan lists the exact output criteria. Do not declare the plan complete until you have seen real output meet them — never fake a verification, and never downgrade this step to "reported as pending".
